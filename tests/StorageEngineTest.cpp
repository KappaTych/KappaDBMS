...
//  se::StorageEngine::getInstance().insert("hehe", "hih");
  se::StorageEngine::getInstance().insert("test_problem", "9~hih");
...
//  std::cout << se::StorageEngine::getInstance().show_create("test_problem") << std::endl;
  std::cout << (se::StorageEngine::getInstance().select("test")).dump();
  std::cout << (se::StorageEngine::getInstance().select("test_problem")).dump();
  return 0;
}
